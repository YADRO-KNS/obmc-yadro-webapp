...
class RedfishNode:
    ...
    # Load configs
    @staticmethod
    def __load_file(filename, schema_path=__BASE_PATH__):
        with open(schema_path + "/schemas/redfish/" + filename) as f:
            data = f.read()
            return data
    ...
    def _properties_expand(self, source: str):
        if not self.__check_properties_definition(source):
            return []
        schema = self.__available_schema()
        id = self.schema_id()
        return Property.build(source, id, schema, self.__get()["Properties"])
...
class DynamicRedfishNode(RedfishNode):
    ...
    def _build_id(self):
        if self._parent is not None:
            ...
            self._id = "/redfish/"
        self._id = self._id + ("/%s{%s}" % (self.uri_prefix(), self.node_parameter().name()))
    ...
    def parameter_template(self):
        if type(self.node_parameter()) == StaticNodeParameter:
