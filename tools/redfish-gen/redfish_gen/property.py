...
class Property():
    @staticmethod
    def build(source, id, schema, definition: dict, global_spec=None):
        kind_property = {
        ...
        return [kind_property(field, id, schema, global_spec) for field in source_def]
    ...
    def __init__(self, definition, schema_name, spec, global_spec=None, field_name=None):
        ...
        if schema_name not in spec:
            if "definitions" in spec:
                ...
                raise ExtractError("Unkown type schema specified: %s" % schema_name)
        ...
        if self.field() in target_spec:
            ...
            raise ValueError("Invalid property specification: %s" % self.field())
    ...
    def __resolve_spec_by_ref(ref_path: str):
        if len(ref_path.path) > 0:
            schema = ref_path.path.replace("/schemas/v1/", "").replace(".json", "")
            node = generator.Generator.get_node(schema)
            schema_path = __BASE_PATH__ + "/schemas/redfish/json-schema/"
            if node is not None:
    ...
    @staticmethod       
    def resolve_global_spec(prop_spec, global_spec):
        ...
        if spec is not None:
            ...
            if ref is not None and not ref.endswith("/idRef"):
                ...
                return Property.resolve_global_spec(ref_spec, spec)
            return spec
        return global_spec if global_spec is not None else prop_spec
    ...
    def resolve_property_spec(prop_spec, global_spec, recursive=True):
        ...
        if recursive and ref is not None:
            ref_spec = Property.__resolve_spec_by_ref(urlparse(ref))
    ...
    def resolve_type(property_spec):
        if "type" in property_spec:
            if property_spec["type"] == "string" and "enum" in property_spec:
                return "enum"
            if type(property_spec["type"]) == list:
    ...
    def is_nullable(self):
        return (type(self._prop_spec["type"]) == list and "null" in self._prop_spec["type"]) or ("anyOf" in self._prop_spec)
...
class StaticProperty(Property):
    def __init__(self, definition, schema_name, spec, global_spec=None):
        super(StaticProperty, self).__init__(definition, schema_name, spec, global_spec)
...
class EntityProperty(Property):    
    ...
    def __init__(self, definition, schema_name, spec, global_spec=None):
        super(EntityProperty, self).__init__(definition, schema_name, spec, global_spec)
    ...
    def _resolve_array_subtype(self, global_spec=None):
        ...
        array_spec = Property.resolve_property_spec(self._prop_spec, global_spec)
        array_type = Property.resolve_type(array_spec)
        if array_type == 'array':
            items = self._prop_spec["items"]
            global_spec =  self._global_spec
            if global_spec is None and 'anyOf' in items:
                global_spec = Property.resolve_global_spec(self._prop_spec, self._global_spec)
                return self._resolve_array_subtype(global_spec)
            array_type = items[0]['type'] if type(items) is list else items['type']
        if type(array_type) == list:
...
class EntityCondition:
    ...
    def __value(self):
        if self._value is None:
            return None
        if type(self._value) == str and self._value.find("::") == -1:
    ...
    def __args(self) -> str:
        ...
        if self._args is None or type(self._args) != list:
            return ""
...
class EntitySource():
    def __init__(self, entity, schema_name, spec, global_spec=None):
        ...
        self._fields = [EntityProperty(field, schema_name,  spec, global_spec) for field in fields_to_init]
...
class Fragments:
    ...
    def _resolve_property_spec(self, spec, global_spec):
        try:
            ...
            raise ExtractError("Could not resolve fragment of %s" % self.name())
    ...
    def _resolve_global_spec(self, ref_spec, global_spec):
        try:
            ...
            return Property.resolve_global_spec(ref_spec, global_spec)
        except:
            raise ExtractError("Could not resolve fragment of %s" % self.name())
...
class Collection(Fragments, EntitySource):
    ...
    def base_class(self, is_parameterized, parent_entity=None):
        if parent_entity is None:
            parent_entity = self.source()
        if is_parameterized:
            parent_entity = "TParameterEntity"  
        return "CollectionGetter<%sFragmentGetter, %s, %s>" % (self.name(), self.source(), parent_entity)
        
class OemFragment(Fragments):
    def __init__(self, definition, parent_id, _, global_spec=None):
        self._oem_name = definition["Name"]
        super(OemFragment, self).__init__(definition, parent_id, None, global_spec, parent_id)
    ...
    def __load_spec(self):
        filename = "%s.v%s.json" % (self.oem_name(), self.version().replace(".", "_"))
        schema_path = "%s/schemas/redfish/json-schema/%s" % (__BASE_PATH__, filename)
        try:
    ...
    def __resolve_property_spec(self, spec, parent_id):
        try:
            ...
            raise ExtractError("Could not resolve fragment of %s" % self.oem_name())
...
class EnumDefinition(Property):
    def __init__(self, definition, schema_name, spec, global_spec=None):
        ...
        super(EnumDefinition, self).__init__(definition, schema_name, spec, global_spec, self.target())
        if global_spec is None:
            global_spec = spec
        self._prop_spec = Property.resolve_property_spec(self._prop_spec, global_spec)
    ...
    def description(self, value):
        if value not in self.values():
            raise LookupError("Value '%s' is not a valid enum of type '%s'" % (value, self.name()))
...
class Links(Property):
    def __init__(self, definition, _, spec, global_spec=None) -> None:
        ...
        self._parameters = [LinkParameter(param_def) for param_def in params_def]
    ...
    def _verify_template(self, spec):
        ...
        if not self.template() in spec_node["uris"]:
            raise ValueError("Invalid template(%s) of link(%s) specified" % (self.template(), self.name()))
...
class RelatedItems(Links):
    def __init__(self, definition, schema_name, spec, global_spec=None) -> None:
        ...
        super(RelatedItems, self).__init__(definition, schema_name, spec, global_spec)
