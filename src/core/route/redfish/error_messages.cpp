...
void resourceNotFound(const RedfishContextPtr& context, const std::string& arg1,
                      const std::string& arg2)
{
    context->getResponse()->setStatus(http::statuses::Code::NotFound);
    context->getResponse()->addError(resourceNotFound(arg1, arg2));
}
 ...
 * @endinternal
 */
nlohmann::json uriNotFound(const std::string& arg1)
{
    return nlohmann::json{
        {"@odata.type", "#Message.v1_1_1.Message"},
        {"MessageId", "Base.1.8.1.ResourceNotFound"},
        {"Message",
         "The requested resource on the URL " + arg1 + " was not found."},
        {"MessageArgs", {arg1}},
        {"MessageSeverity", "Critical"},
        {"Resolution",
         "Provide a valid resource URI and resubmit the request."}};
}
...
void uriNotFound(const RedfishContextPtr& context)
{
    context->getResponse()->setStatus(http::statuses::Code::NotFound);
    context->getResponse()->addError(
        uriNotFound(context->getRequest()->getUriPath()));
}
