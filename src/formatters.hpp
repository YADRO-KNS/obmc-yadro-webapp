...
static const DbusVariantType formatValueFromDict(
    const std::map<TRawValue, TResolvedValue>& formatterDict,
    ...
    const TResolvedValue defaultValue)
{
    auto formattedValue = std::visit(
        [propertyName, &formatterDict,
         defaultValue](auto&& property) -> const DbusVariantType {
...
class TrimFormatter : public query::dbus::IFormatter
{
  public:
                                 ...
                                 const DbusVariantType& value) override
    {
        if (!std::holds_alternative<std::string>(value))
        {
            throw std::invalid_argument("The property '" + property +
